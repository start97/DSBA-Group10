...
First, we need to merge the datafiles of internet access and crimes per CSA's into one dataset, to generate meaningful output of it. Here we simply merge the files by their same column, namely the community statistical area. 

```{r echo=TRUE}
Percent_of_Households_with_No_Internet_at_Home <- mutate(Percent_of_Households_with_No_Internet_at_Home, =tolower(CSA))


Joined_Poverty_Crime_dataset <- crime_data_with_areas %>% left_join(Percent_of_Households_with_No_Internet_at_Home,by="CSA")
```
