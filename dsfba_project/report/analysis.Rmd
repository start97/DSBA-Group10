...
MultipleRegression1 <- lm(Community_data$CrimePer1000inhabitants~Community_data$density_perc+Community_data$hhpov19)
...
anova(MultipleRegression1)

SSE1 <- sum((fitted(MultipleRegression1)-Community_data$CrimePer1000inhabitants)^2)

SST1 <- sum((Community_data$CrimePer1000inhabitants-mean(Community_data$CrimePer1000inhabitants))^2)

SSR1 <- SST1-SSE1

df1 <- 2

df2 <- nrow(Community_data)-2-1
...
Fborder1 <- qf(0.05,df1,df2, lower.tail=FALSE)

Fstat1>Fborder1 #As it returns TRUE, we know that it falls in the rejection area, that we can reject the null hypothesis and that the overall regression  is significant

#Rsquared and Adjusted Rsquared

summary(MultipleRegression1)$r.squared 
summary(MultipleRegression1)$adj.r.squared 

#We have a decent results

#From the summary of the regression we see that the regression coefficient for poverty is not significant.

#One further way to discuss about the model quality is associated to the standard error of the estimate:

MultipleRegression1$fitted.values
Community_data$CrimePer1000inhabitants
...
library(car)
vif(MultipleRegression1) #The VIF is low, indicating a rather low multicollinearity, which is what we expect
