...
First, we create a CCTV_VS_crimes data set (which basically is a left joint). Next, we are able to plot visually compare the crime rate per area with the density percentage per area. Here we see that there seems to be a trend. 
...
Next, we decided to calculate the crime rate for August per area to see where crime was highest. The results show that they are in midtown (are midtown). 
