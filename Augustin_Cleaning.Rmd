...
CrimeCategoryRepartition <- CrimePerCategoryPerArea %>% 
  group_by(Category) %>% 
  summarise(Repartition=sum(CrimePerCategoryPerArea)) #We observe that in Baltimore, the number of felony is close to the number of misdemeanor

FelonyStats <- filter(CrimePerCategoryPerArea,Category=="Felony")

FelonyRatePerArea <- FelonyStats$RepartitionPerCategoryPerArea/CrimeCateogryRepartition$Repartition[1]*100

FelonyStats <- cbind(FelonyStats,FelonyRatePerArea=FelonyRatePerArea)

sum(FelonyStats$FelonyRatePerArea)

MisdemeanorStats <- filter(CrimePerCategoryPerArea,Category=="Misdemeanor") %>% 
  mutate(MisdemeanorStats,MisdemeanorRatePerArea=RepartitionPerCategoryPerArea/CrimeCateogryRepartition$Repartition[2])
