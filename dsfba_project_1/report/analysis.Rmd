...
ggplot(data=CCTV_VS_misdemeanors,mapping= aes(x=density_perc,y=MisdemeanorPerCapitaPerArea)) + 
  ...
  geom_smooth(method = lm,color="blue",size=0.3)
```
We are in the exact same situation as with crime, in order to comment CCTC effectiveness on felonies and/or misdemeanors, we must analyse felony/misdemeanor evolution over time in areas with/without CCTVs. We start with felonies. 
...
Downtown_Seton_Hill_evolution_Felony <- as.vector(t(Community_data[14,c(79,77,75,73,71,69,67)]))

Downtown_Seton_Hill_Felony <- data.frame(Downtown_Seton_Hill_evolution_Felony,Year)

ggplot(Downtown_Seton_Hill_Felony,aes(x=Year,y=Downtown_Seton_Hill_evolution_Felony)) + 
  geom_line() +
  scale_y_continuous(limits = c(0, 125), breaks =seq(0,125,25)) + 
  labs(title = "Felony has slightly increased in Downtown/Seton Hill",subtitle=str_wrap(("One observe variation from 2014 to 2019 but overall, felony has increased."),width=80),x="Year",y="Felony per capita")
...
Sandtown_Winchester_Harlem_Park_evolution_Felony <- as.vector(t(Community_data[47,c(79,77,75,73,71,69,67)]))

Sandtown_Winchester_Harlem_Park_Felony <- data.frame(Sandtown_Winchester_Harlem_Park_evolution_Felony,Year)

ggplot(Sandtown_Winchester_Harlem_Park_Felony,aes(x=Year,y=Sandtown_Winchester_Harlem_Park_evolution_Felony)) + 
  geom_line() +
  scale_y_continuous(limits = c(0, 125), breaks =seq(0,125,25)) + 
  labs(title = str_wrap(("Felony is fairly stable in Sandtown-Winchester/Harlem Park"),width = 60),subtitle=str_wrap(("After a slight increase in 2015, felony slowly decreased until 2019."),width=80),x="Year",y="Felony per capita")
...
Oldtown_Middle_East_evolution_Felony <- as.vector(t(Community_data[41,c(79,77,75,73,71,69,67)]))

Oldtown_Middle_East_Felony <- data.frame(Oldtown_Middle_East_evolution_Felony,Year)

ggplot(Oldtown_Middle_East_Felony,aes(x=Year,y=Oldtown_Middle_East_evolution_Felony)) + 
  geom_line() +
  scale_y_continuous(limits = c(0, 125), breaks =seq(0,125,25)) + 
  labs(title=str_wrap(("Felony slightly increased in Oldtown/Middle East"),width=60),subtitle=str_wrap(("After an increase over the 2014-2017 period, felony decreased in 2018 before increasing again."),width=80),x="Year",y="Felony per capita")
...
Cherry_Hill_evolution_Felony <- as.vector(t(Community_data[7,c(79,77,75,73,71,69,67)]))

Cherry_Hill_Felony <- data.frame(Cherry_Hill_evolution_Felony,Year)

ggplot(Cherry_Hill_Felony,aes(x=Year,y=Cherry_Hill_evolution_Felony)) + 
  geom_line() +
  scale_y_continuous(limits = c(0, 125), breaks =seq(0,125,25)) + 
  labs(title = str_wrap(("Overall, felony did not change much in Cherry Hill"),width = 60),subtitle=str_wrap(("Still, it peaked in 2016 before going back to a level comparable to that of 2014 in 2019."),width=80),x="Year",y="Felony per capita")
