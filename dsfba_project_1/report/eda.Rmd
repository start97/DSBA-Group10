...
Prison_map <- tm_shape(Prison_area) + tm_borders(col="black",alpha=0.3)+ tm_shape(baltimore) + tm_fill(col = "CrimePer1000inhabitants", title ="Crime per capita",style = "quantile") + tm_borders(col="black") + tm_layout(inner.margins = 0.05,frame.lwd = 5,main.title = "We have no data about crime committed in prison, this explains this crime-free zone.",main.title.position = c('left', 'top'),main.title.size = 1)+tm_scale_bar(position = c("left", "top"))+ tm_shape(balt_dat) + tm_dots(col="black") #This map zooms on the prison. This "Area" is special. We have no data on crime there, we can also see that the there is a huge concentration of CCTVs directly next to the prison.
...
ggplot(data=Felony_VS_Misdemeanor,mapping= aes(x=FelonyPerCapitaPerArea,y=MisdemeanorPerCapitaPerArea)) + 
  labs(title = "Misdemeanors and felonies are rather equally distributed in Baltimore", x="Felony (per 1000 inhabitants)",y="Misdemeanor (per 1000 inhabitants)")+
...
library(magick)
library(av)
tmap_animation(animated_crime_map,filename = "testmp4.mp4",delay=100)

magick::image_read("animated_crime_map.gif")

```

<div align="center">
<img src="animated_crime_map.gif" width="180" height="105" name="animated_crime_map"> 
</div>
