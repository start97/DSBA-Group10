...
Crime_Monthly_evolution_map <- crime_data_with_areas %>% 
  ...
  labs(xlab="Year",ylab="Crime occurences",title = "Crime seasonality observation: crime tends to decrease in winter.")
...
Violent_Crime_Yearly_evolution_map <- crime_data_with_areas %>%
  ...
  labs(title = str_wrap(("Overall, violent crime seems to have decreased for the 2017 to 2020 period"),width=65),x="Year",y="Violent crime occurences")
...
ggplot(data=Crime_VS_Poverty,mapping= aes(x=hhpov19,y=CrimePer1000inhabitants)) + 
  labs(title = "Poorer areas tend to be more impact by crime", x="Poverty rate",y="Crime per Capita")+
  geom_point() + 
  geom_smooth(method = lm,color="blue",size=0.3)
